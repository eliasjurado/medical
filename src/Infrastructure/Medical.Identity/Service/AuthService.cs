...
namespace Medical.Identity.Service
{
    public class AuthService : IAuthService
    {
        ...
        public async Task<IResponse> Login(UserLogin request)
        {
            ...
            if (user == null)
            {
                return new DataResponse<AuthResponseDto>(new AuthResponseDto(), HttpStatusCodes.NOT_FOUND, $"User with {request.Email} not found.", false);
            }
        ...
        public async Task<IResponse> RefreshToken(RefreshTokenRequest request)
        {
            ...
            {
                return new DataResponse<AuthResponseDto>(new AuthResponseDto(), HttpStatusCodes.NOT_FOUND, String.Format(Messages.NotFound, "Refresh Token"), false);
            }
            ...
            if (user == null)
            {
                return new DataResponse<AuthResponseDto>(new AuthResponseDto(), HttpStatusCodes.NOT_FOUND, $"No User found with token.", false);
            }

            if (ValidateToken(request.CurrentToken))
            {
                return new DataResponse<AuthResponseDto>(new AuthResponseDto(), HttpStatusCodes.NOT_FOUND, Messages.TokenNotExpired, false);
            }
        ...
        public async Task<IResponse> Register(UserRegister request)
        {
            ...
            if (result.Succeeded)
            {
                ...
                return new DataResponse<string>(user.Id, HttpStatusCodes.ACCEPTED, Messages.REGISTRATION_SUCCESS);
            }
            else
            {
                ...
                return new DataResponse<string?>(null, HttpStatusCodes.BAD_REQUEST, str, false);
            }
        }
